...
class SitemapReducer {
  ...
  reduce(cache) {
    ...
    this.appTypes.forEach(appType => {
      ...
      urls.push(baseRoute);
      const platforms = findMetadataFiles(resolve(this.articlesDir, appType.slug), 'index.yml');
    ...
    // Add all public non-quickstart documents, as long as they are not marked as excluded.
    cache.forEach(doc => {
