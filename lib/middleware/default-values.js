...
var cards = yaml.safeLoad(fs.readFileSync('./config/cards.yml'));
var categories = yaml.safeLoad(fs.readFileSync('./config/navigation.yml'));
  ...
  if (Array.isArray(obj)) {
    ...
    for (var key in obj) {
      ...
      if (Array.isArray(val)) {
        ...
        fixLinks(val);
      } else if (key === 'href' && val.indexOf('/') === 0) {
        obj[key] = '/docs' + val;
      }
    }
  }
...
export default function defaultValues(req, res, next) {
  ...
  res.locals.navigation = { categories, cards, platforms };
  ...
  let footerView = path.resolve(__dirname, '../../node_modules/auth0-styleguide/lib/footer/demo.jade');
  jade.renderFile(footerView, (err, html) => {
    res.locals.webfooter = html;
    next();
  });
