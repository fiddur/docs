...
module.exports = function (app) {
  ...
  // Connections API
  app.get(nconf.get('BASE_URL') + '/api/connections', json.bind(json, { connections: connections }));
  app.get(nconf.get('BASE_URL') + '/api/connections/:connectionType', json.bind(json, function(req) {
    return { connections: _.filter(connections, { 'type': req.params.connectionType }) };
  }));
  app.get(nconf.get('BASE_URL') + '/api/connections/:connectionType/:hash', json.bind(json, function(req) {
    return _.find(connections, { 'type': req.params.connectionType, 'hash': req.params.hash });
  }));

  // Article Collections API
  app.get(nconf.get('BASE_URL') + '/api/articles', json.bind(json, { articles: articles.items }));
  app.get(nconf.get('BASE_URL') + '/api/articles/*', json.bind(json, function(req, res) {
    var parts = req.params[0].replace('/api/articles/').split('/');
