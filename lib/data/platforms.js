...
appTypes.forEach(appType => {
  ...
  indexes.forEach(doc => {
    ...
    let createArticleEntry = (name, index) => {
      ...
      return {
        name,
        number: index === false ? undefined : index + 1,
        title: doc.title,
    ...
    platform.articles = meta.articles.map(createArticleEntry);

    if (meta.default_article) {
      platform.defaultArticle = createArticleEntry(meta.default_article, false /* don't add article number */);
    }
