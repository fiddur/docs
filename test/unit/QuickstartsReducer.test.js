...
describe('QuickstartsReducer', () => {
  ...
  describe('when reduce() is called', () => {
    ...
    it('returns an object a key for each app type', () => {
      expect(Object.keys(result)).to.deep.equal(['example-apptype']);
    });

    it('merges the metadata in from the app type', () => {
      const appType = appTypes[0];
      const example = result['example-apptype'];
      expect(example.name).to.equal(appType.name);
      expect(example.title).to.equal(appType.title);
      expect(example.slug).to.equal(appType.slug);
      expect(example.flavor).to.equal(appType.flavor);
    });

    it('builds a hash of platforms for each app type', () => {
      const { platforms } = result['example-apptype'];
    ...
    it('sets basic information on each platform', () => {
      const platform = result['example-apptype'].platforms['platform-a'];
      expect(platform.name).to.equal('platform-a');
      expect(platform.title).to.equal('Platform A');
      expect(platform.url).to.equal(urlFormatter.format('/quickstart/example-apptype/platform-a'));
    ...
    it('adds a list of articles to each platform', () => {
      const platformA = result['example-apptype'].platforms['platform-a'];
      const platformB = result['example-apptype'].platforms['platform-b'];
    ...
    it('sets basic information on each article', () => {
      const article = result['example-apptype'].platforms['platform-a'].articles[0];
      expect(article.name).to.equal('01-example');
      expect(article.number).to.equal(1);
      expect(article.url).to.equal(urlFormatter.format('/quickstart/example-apptype/platform-a/01-example'));
    });
