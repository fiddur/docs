  ...
  highlight: function(str, lang) {
    if (lang === 'har') {
      ...
      try {
        ...
        return httpSnippetTemplate({ snippets: snippets });
      } catch (ex) {
...
markdown.renderer.rules.heading_open = function(...args) {
  ...
  html += '<span id="' + headingIndex + '" class="anchor">' +
...
markdown.renderer.rules.heading_close = function(...args) {
  const [ tokens, idx, , env, self ] = args;

  var html = '</a>';
  if (originalHeadingClose) {
    html += originalHeadingClose.apply(this, args);
  } else {
    html += self.renderToken(...args);
  }
  return html;
