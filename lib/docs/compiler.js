...
var markdown = markdownIt({
  ...
  highlight: function(str, lang) {
    if (lang === 'har') {
      ...
      try {
        // First convert the JSON object to valid har
        if (har.postData && har.postData.json) {
          var json = har.postData.json;
          delete har.postData.json;
          var text = JSON.stringify(json);
          har.postData.text = text;
        }
        ...
        httpTargets.map(target => {
          ...
          // Some random encoding issue. ¯\_(ツ)_/¯
          code = code.replace(/https\:\/\/\//g, 'https://');
          // Escape HTML
          code = markdown.utils.escapeHtml(code);
          // Brackets { } are TOO escaped, unescape them a bit ¯\_(ツ)_/¯
          code = code.replace(/%7B/g, '&#123;').replace(/%7D/g, '&#125;');
...
var panelGenerator = function(level) {
  ...
  return {
    ...
    render: function (tokens, idx) {
      ...
      if (tokens[idx].nesting === 1) {
        // opening tag
        return `<div class="panel panel-${level}" data-swiftype-index="false">` +
