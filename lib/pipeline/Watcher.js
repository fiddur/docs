...
class Watcher extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.getFilePath = options.getFilePath || Watcher.defaults.getFilePath;
   ...
   * @param {String} relativeFilename - The filename of the file, relative to the baseDir.
   * @param {String} [encoding] - The file's encoding, or UTF8 if not specified.
   */
  load(relativeFilename, encoding = 'utf8') {
    const filename = resolve(this.baseDir, relativeFilename);
    const path = relative(this.baseDir, filename).replace(extname(filename), '').replace(/\/index$/, '');
...
Watcher.defaults = {
  ...
  getFilePath: (filename) => filename.replace(extname(filename), '').replace(/\/index$/, '')
};
