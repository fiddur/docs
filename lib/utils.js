...
var quickstart = yaml.safeLoad(fs.readFileSync(__dirname + '/../docs/quickstart.yml'));
...
function getSubdomain(wildcard) {
  if (wildcard.split('.').length < 3) { return wildcard; }
  var matches = wildcard.match(/^(https?:\/\/)[\w|\*\-]+\.?(.*)/);
  if (matches === null) { return wildcard; }
  return matches[1] + matches[2];
}

utils.equalBaseUrls = function (url1, url2) {
  if (!url1 || !url2)
    return false;
...
utils.alternative_title = function (content) {
  var regex = /\#{1}[^\n\#]+/g;
...
utils.loadYaml = function(ymlFilePath) {
  var fullPath = path.join(__dirname, ymlFilePath);
