...
export default function createPipeline() {
  ...
  const urlFormatter = new UrlFormatter({
    baseUrl: nconf.get('DOMAIN_URL_DOCS').replace('/docs', ''),
  ...
  const watcher = new Watcher({ baseDir: nconf.get('DOCS_PATH') });
  const cache = new Cache({ compiler, watcher });
  ...
  cache.whenReady(() => {
    debug('Initial scan complete. Creating reducers...');
