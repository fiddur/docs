...
export default function bootstrap(options, req, res, next) {
  ...
  const { framed, fullWidth, singleQuickstart, staticPage } = res.locals;

  const context = app.createContext({ debug: process.env.NODE_ENV !== 'production' });
  const env = Object.assign({}, vars.env);
  const flags = { framed, fullWidth, singleQuickstart };
  ...
  plugin.registerService('ArticleService', createArticleService(req, res));
