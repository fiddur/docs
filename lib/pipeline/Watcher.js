...
/**
 * Scans the filesystem for changed files and monitors them, emitting events
...
class Watcher extends EventEmitter {
   ...
   * @param {Boolean} [options.monitorContent] - If true, force content monitoring.
   */
  constructor(options) {
    super();
    ...
    // Only watch the filesystem if it has been specifically requested. Otherwise, we'll
    // stop after the initial scan.
    const persistent = options.monitorContent || process.env.MONITOR_DOCS_CONTENT === 'true';

    // If we need to monitor and we aren't on MacOS, we won't have fsevents,
    // so we have to fall back to polling the filesystem.
    const usePolling = persistent && process.platform !== 'darwin';
  ...
  /**
   * Causes the Watcher to stop monitoring files.
   */
  stop() {
    this.watcher.close();
  }
  ...
  /**
   * Called when chokidar completes its initial scan.
   * @private
   */
  handleReady() {
    this.emit('ready');
  ...
  /**
   * Called when chokidar discovers a new file.
   * @param {String} path - The path (relative to the baseDir) of the discovered file.
   * @private
   */
  ...
  /**
   * Called when chokidar detects a change in a monitored file.
   * @param {String} path - The path (relative to the baseDir) of the changed file.
   * @private
   */
