...
function renderApp(req, res, next) {
  ...
  .then(context.executeAction(loadContent, { url: req.url, category: 'articles', ignoreMissing: true }))
...
function renderError(err, req, res, next) {
  ...
  .then(() => {
    ...
    let props = {
      ...
      className:     (req.url == '' || req.url == '/') ? 'docs-home' : 'docs-single',
      state:         `window.App=${serialize(app.dehydrate(context))};`,
      markup:        ReactDOMServer.renderToString(createElementWithContext(context, {currentNavigateError: err})),
    };
