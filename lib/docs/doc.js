...
import { comnpileTemplate, makeLinksAbsolute } from './compiler';
...
function Doc(docsPath, filename){
  ...
  var isMarkdown = /\.md$/.test(filename);

  //TODO: implement section files
  var template;
  try {
    template = comnpileTemplate(content, isMarkdown);
  } catch (ex) {
    ex.message = 'Error compiling document "' + filename + '"';
    throw ex;
  }
...
Doc.prototype.processSections = function(context, absoluteLinks) {
  var sections = {};
