...
function apiExplorerV1(options, req, res) {
  ...
  // NOTE: API Explorer v1 is always in read-only mode now
  // as we no longer want to expose the global client secret
  // publically due to security concerns.
  ctx.readOnly = true;
  ctx.user = {
    id: 'john.doe',
    name: 'John Doe',
...
router.get('/api/v1', apiExplorerV1.bind(this, {title: 'Management API v1 (deprecated)', isAuth: false, anchors: true}));
