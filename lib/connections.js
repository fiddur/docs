...
function findByTicket(ticket, callback) {
  getDb(function(db) {
...
function getCurrentStep(ticket, callback) {
  /*
  ...
  getDb(function(db) {
    findByTicket(ticket, function(err, connection) {
      ...
      if (!connection.options.thumbprints) return callback(null, {
        currentStep: 1
      });
      ...
      db.collection('users').find({
        ...
        if (userCount === 0) return callback(null, {
          currentStep: 2
        });

        if (userCount === 1) return callback(null, {
          currentStep: 3
        });
