...
class ReplaceIncludesPlugin {
  ...
  transform(meta, content) {
    ...
    while (match = regex.exec(content)) { // eslint-disable-line no-cond-assign
      ...
      const include = this.evaluate(code);
      const filename = this.resolveInclude(meta.filename, include.path);
      const rendered = this.renderInclude(filename, meta, include.vars);
