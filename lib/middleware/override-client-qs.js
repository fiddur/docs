...
export default function overrideIfClientInQs(req, res, next) {
  ...
  clients.find(params, function(err, clients) {
    ...
    res.locals.account.guardianNamespace = regions.get_guardian_namespace(req.user.tenant.region).replace('{tenant}', req.user.tenant.name);
    ...
    res.locals.account.callback = client.callback || res.locals.account.callback;
    res.locals.connectionName = req.query.conn || 'YOUR_CONNECTION_NAME';
