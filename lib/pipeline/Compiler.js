...
class Compiler {
  ...
  compile(file) {
    try {
      ...
      // Prepare the hash of variables that will be available in the template.
      const context = merge({}, this.vars, doc);

      // Render the content and store it on the document.
      doc.content = template(context);
