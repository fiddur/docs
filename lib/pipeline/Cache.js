...
class Cache extends EventEmitter {
   ...
   * @param {String} options.urlFormatter - A UrlFormatter instance.
  ...
  constructor(options = {}) {
    ...
    assert(options.urlFormatter, 'Cache constructor requires a urlFormatter option');
    ...
    // Ensure that the reductions aren't recalculated more than once every 500ms.
    // (This can happen if several documents are changed at once, resulting in several
    // change events coming from the Watcher.)
    this.processReductions = debounce(this.processReductions.bind(this), 500);
  }
  ...
  /**
   * Return the document in the cache with the specified (absolute) filename, or
   * throws an Error if no such document exists.
   * @param {string} filename - The filename to match.
   * @returns {Document} The document, or undefined if no match was found.
   */
  getByFilename(filename) {
    const doc = this.tryGetByFilename(filename);
    if (!doc) throw new Error(`No document with the filename ${filename} exists in the cache`);
    return doc;
  }
   ...
   * @returns {Document} The document, or undefined if no match was found.
   */
  tryGetByFilename(filename) {
    return this.docsByFilename[filename];
  }

  /**
   * Return the document in the cache with the specified URL (which is first
   * normalized using the UrlFormatter), or throws an Error if no such document exists.
   * @param {string} url - The URL to match.
   * @returns {Document} The document, or undefined if no match was found.
   */
  getByUrl(url) {
    const doc = this.tryGetByUrl(url);
    if (!doc) throw new Error(`No document with the URL ${url} exists in the cache`);
    return doc;
  }

  /**
   * Return the document in the cache with the specified URL (which is first
   * normalized using the UrlFormatter.)
   * @param {string} url - The URL to match.
   * @returns {Document} The document, or undefined if no match was found.
   */
  tryGetByUrl(url) {
    return this.docsByUrl[this.urlFormatter.format(url)];
  }
   ...
   */
  handleWatcherReady() {
    debug(`[ready] watcher scan complete, found ${this.getStats().count} documents`);
    this.processReductions();
    this.isReady = true;
...
Cache.defaults = {
  isArticle: (file) => (
    ...
    ['.md', '.html'].indexOf(extname(file.shortname)) !== -1 &&
