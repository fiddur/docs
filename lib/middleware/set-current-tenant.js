...
export default function setCurrentTenant(req, res, next) {
  var done = function() {
    ...
    logger.info('Current tenant is: ' + req.user.tenant.name + ' region: ' + req.user.tenant.region);
    logger.info('Region list', req.user.regions);
  ...
  authorizations.getRegionsAndTenants(req.user.id, function(err, regions) {
    if (err) {
      logger.error('get user tenants error: ' + err.message);
