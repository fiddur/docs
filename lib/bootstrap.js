...
import loadUserAction from '../client/action/loadUserAction';
...
export default function bootstrap(options, req, res, next) {
  ...
  .then(() => {
    if (req.user) {
      return context.executeAction(loadUserAction, {
  ...
  .then(() => {
    // If we are using static content, no doc will be present so we load metadata and static content
    if (staticPage) {
      env.staticPage = true;
      env.fullWidth = staticPage.fullWidth || false;
      pageTitle = staticPage.pageTitle;
      pageDescription = staticPage.pageDescription;
      return context.executeAction(loadStaticContent, staticPage.html);
    }
    return null;
  })
  .then(() => {
    let markup;
    if (staticPage) {
      markup = ReactDOMServer.renderToStaticMarkup(createElementWithContext(context))
    } else {
    ...
    let html;
    if (staticPage) {
      html = ReactDOMServer.renderToStaticMarkup(createLayoutComponent(layoutProps));
    } else {
      html = ReactDOMServer.renderToString(createLayoutComponent(layoutProps));
    }
