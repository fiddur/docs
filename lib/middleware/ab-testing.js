...
function doFetch (callback) {
  winston.info('Purging experiments cache');
  cache = null;

  winston.info('Fetching experiments from server');
  ...
  request.get(requestOptions, function(error, response, body) {
    if (!error && response.statusCode === 200) {
      winston.info('Experiments fetched. Executing callback');
...
export default function fetchABExperiments (req, res, next) {
  winston.info('Loading Experiments');

  if (!experimentsEndpoint) {
    winston.warn('Experiments endpoint is not defined.');
    res.locals.abExperiments = null;
    return next();
  }

  if (cache) {
    winston.info('Returning cached experiments.');
  ...
  if (retryBackOff) {
    winston.warn('Retrying in progress. Skipping experiments fetch.');
    res.locals.abExperiments = null;
    return next();
  }
