...
import LayoutAmp from '../client/components/Layout.amp';
...
let LayoutAmpComponent = React.createFactory(LayoutAmp);
...
function renderAmp(req, res, next) {
  ...
  contentService.load(url).then(markup => {
    contentService.getMeta(url).then(meta => {
      let props = {
        pageTitle:       meta.title || 'Auth0',
        markup:          markup
      };

      let html = ReactDOMServer.renderToStaticMarkup(LayoutAmpComponent(props));
      html = html.replace('<html ', '<html amp ');
      html = html.replace('<head>', '<head><meta charset="utf-8">');
      html = html.replace('<style>', '<style amp-boilerplate>');
      html = html.replace('<style>', '<style amp-boilerplate>');
      res.type('html');
...
handlers.content = (req, res, next) => {
  ...
  }  else if (res.locals.amp) {
    return renderAmp(req, res, next);
  } else {
