...
function buildDoc(file) {
  ...
  var templates = {};
  try {
    templates.html = compileTemplate(content, isMarkdown);
    templates.amp = compileTemplate(content, isMarkdown, true /* AMP */);
  } catch (ex) {
  ...
  doc.render = function(locals) {
    if (locals.amp) {
      return this.templates.amp(locals);
    } else {
      return this.templates.html(local);
    }
  }
