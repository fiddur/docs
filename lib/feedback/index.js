...
if (nconf.get('SLACK_WEBHOOK')) {
  slackClient = new SlackClient(nconf.get('SLACK_WEBHOOK'));
}

function postFeedbackToSlack(feedback, user) {
  if (!slackClient) {
    logger.warn('Slack client is not intialized.');
    return;
  }
  ...
  slackClient.postSlackMessage(message).catch(logger.error);
}
...
function postFeedbackToZendesk(feedback, user) {
  if (!zendeskClient) {
    logger.warn('Zendesk client is not intialized.');
    return;
  }

  if (!feedback.comment) {
    logger.info('Ticket not posted to zendesk, no comment provided.');
    return;
  }

  logger.info('Posting feedback to zendesk');
...
function postFeedbackToGithub(feedback, user) {
  if (!githubClient) {
    logger.warn('Github client is not intialized.');
    return;
  }
