...
function renderDoc(req, res, next) {
  ...
  // TODO: Should we remove noRender, or should we make it compatible with embedded mode?
  //} else if (res.locals.embedded && !res.locals.noRender) {
  } else if (res.locals.embedded) {
    viewname = (doc.meta.layout || 'doc-embedded') + '.jade';
    res.locals.sections = processSections(doc, res.locals, true /* absolute links */);
    res.render(viewname, {});
  } else if (res.locals.framed && !res.locals.noRender) {
    viewname = (doc.meta.layout || 'doc-framed') + '.jade';
    res.locals.sections = processSections(doc, res.locals, true /* absolute links */);
    res.render(viewname, {});
  } else if (!res.locals.noRender) {
