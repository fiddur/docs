...
import AutoQuickstartPlugin from './plugins/metadata/AutoQuickstartPlugin';
...
import AutoVersionPlugin from './plugins/metadata/AutoVersionPlugin';
...
export default function createPipeline() {
  ...
  compiler.use(new AutoQuickstartPlugin({ appTypes }));
  compiler.use(new IndexMergePlugin({ baseDir }));
  compiler.use(new NormalizeUrlsPlugin({ urlFormatter }));
  compiler.use(new AutoVersionPlugin());
  ...
  const cache = new Cache({
    ...
    reducers: {
      ...
      snippets: new SnippetsReducer({ snippetsDir, vars }),
      updates: new UpdatesReducer({ updatesDir }),
      versions: new VersionsReducer({ articlesDir, urlFormatter })
    }
  });
