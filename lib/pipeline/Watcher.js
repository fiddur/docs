...
class Watcher extends EventEmitter {
  ...
  constructor(options) {
    ...
    const watcherConfig = {
      cwd: this.baseDir,
      // If we aren't on MacOS, we won't have fsevents, so we have to fall back to polling.
      usePolling: (process.platform !== 'darwin'),
      // In production, don't watch for changes, just do the initial scan.
    ...
    this.watcher = chokidar.watch(null, watcherConfig);
