...
import { compileTemplate, makeLinksAbsolute } from './compiler';
...
function Doc(filepath, text){

  var data = matter(text);
  ...
  if (!this._meta.url) {
    ...
    this._meta.url = '/' + this._filename.replace(/\.[\w]+$/, '').replace(/\/index$/, '');
  }
